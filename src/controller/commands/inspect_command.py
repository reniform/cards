...
from termio.view import ManaColor
...
class InspectCommand(Command):
    ...
    def execute(self, game_state: "GameState") -> tuple[bool, bool]:
        ...
        if isinstance(card_to_inspect, MonsterCard):
            ...
            print(f"  - Type: {card.mana_type.value} | Stage: {card.stage.value} | HP: {card_to_inspect.health}/{card.health}")
            print(f"  - Evolves From: {card.evolve_from or 'N/A'}")
            print(f"  - Weakness: {card.weak_type.value if card.weak_type else 'N/A'} (x{card.weak_mult or '0'})")
            print(f"  - Resistance: {card.resist_type.value if card.resist_type else 'N/A'} (-{card.resist_val or '0'})")
